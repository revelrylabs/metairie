defmodule Metairie.Phoenix.Instrumenter do
  ...
  def phoenix_controller_call(:start, _compile_metadata, runtime_metadata) do
    StatsCollector.increment("phoenix.request.count", 1, tags: tags(runtime_metadata.conn))
    runtime_metadata
  end

  @doc false
  def phoenix_controller_call(:stop, diff, result_of_before_callback) do
    StatsCollector.increment("phoenix.response.count", 1,
      tags: tags(result_of_before_callback.conn)
    )

    StatsCollector.histogram("phoenix.response.time", diff,
      tags: tags(result_of_before_callback.conn)
    )
  end
